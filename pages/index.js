...
const TrashIcon = () => <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 6h18"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/></svg>
...
// Recibe también el valor anterior y la posición del cursor para poder detectar cuando el
// usuario borra el espacio que puso el formateo automático (el espacio nunca queda al final,
// así que sin esto se reponía solo y había que apretar borrar dos veces para sacar la letra de al lado).
function formatPatente(val, valorAnterior, cursorPos) {
  ...
  const p = (base||'').toUpperCase().replace(/[^A-Z0-9]/g,'').slice(0,7)
  return p.length===7 ? `${p.slice(0,2)} ${p.slice(2,5)} ${p.slice(5)}` : p.length===6 ? `${p.slice(0,3)} ${p.slice(3)}` : p
}
// Handlers para un input de patente: mientras el navegador está componiendo un caracter
// (teclados de celular con autocorrección/predictivo, IME) dejamos pasar el valor tal cual,
// sin reformatear — reformatear a mitad de la composición hacía que el teclado del celular
// mezclara las letras (ej. quedaba "AA BAB 1A" en vez de "AB 123 CD"). El formato final se
// aplica recién cuando termina de componer.
...
const autocompleteSyle={position:'absolute',top:'100%',left:0,right:0,background:'#171B25',border:'1px solid #343B4B',borderRadius:'6px',zIndex:100,boxShadow:'0 8px 24px rgba(0,0,0,.4)',maxHeight:'200px',overflowY:'auto'}
const autocompleteItemStyle={padding:'8px 12px',cursor:'pointer',fontSize:'13px',borderBottom:'1px solid #262B38',color:'#EEF1F7'}
  ...
  return (
    <div style={{position:'relative'}}>
      ...
      {sugerencias.length>0 && <div style={autocompleteSyle}>{sugerencias.map(t=>
        <div key={t.id} style={autocompleteItemStyle} onMouseOver={e=>e.currentTarget.style.background='#1C2130'} onMouseOut={e=>e.currentTarget.style.background='transparent'} onClick={()=>seleccionar(t)}>
    ...
    return(
      <div>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'16px'}}>
          ...
          <span style={{fontWeight:'700',fontSize:'18px',color:'#EEF1F7'}}>{MESES[mes]} {anio}</span>
          <button className={styles.btn} style={{padding:'8px 16px',fontSize:'16px'}} onClick={()=>setMesCalendario(new Date(anio,mes+1,1))}>›</button>
        </div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:'6px',marginBottom:'10px'}}>
          {DIAS_SEMANA.map(d=><div key={d} style={{textAlign:'center',fontSize:'12px',fontWeight:'700',color:'#9AA3B8',padding:'6px 0'}}>{d}</div>)}
        </div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:'6px'}}>
          {celdas.map((dia,i)=>{
            ...
            return(
              <div key={i} onClick={()=>abrirDia(f)} style={{minHeight:'72px',borderRadius:'10px',padding:'6px',cursor:'pointer',border:`2px solid ${seleccionado?'#2FA8FF':esHoy?'#F5B700':'#343B4B'}`,background:seleccionado?'rgba(47,168,255,.12)':esHoy?'rgba(245,183,0,.12)':completo?'rgba(255,77,79,.1)':'#1C2130',transition:'all .15s'}}>
                <div style={{textAlign:'center',fontSize:'14px',fontWeight:esHoy?'700':'600',color:esHoy?'#F5B700':completo?'#FF4D4F':'#EEF1F7',marginBottom:'4px'}}>{dia}</div>
                {tDia.length>0&&<div style={{display:'flex',flexDirection:'column',gap:'2px'}}>
                  {tDia.slice(0,3).map((t,j)=><div key={j} style={{fontSize:'10px',background:completo?'rgba(255,77,79,.18)':'rgba(47,168,255,.18)',color:completo?'#FF4D4F':'#2FA8FF',borderRadius:'4px',padding:'2px 4px',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{t.nombre}</div>)}
                  {tDia.length>3&&<div style={{fontSize:'10px',color:'#9AA3B8',textAlign:'center'}}>+{tDia.length-3} más</div>}
                </div>}
                {tDia.length===0&&<div style={{fontSize:'10px',color:'#6B7488',textAlign:'center',marginTop:'6px'}}>libre</div>}
              </div>
            )
          })}
        </div>
...
{checklistActivo&&!editandoChecklist&&<div className={styles.modalOverlay}><div className={styles.modal} style={{width:'100%',maxWidth:'600px',maxHeight:'85vh',overflowY:'auto'}}><div className={styles.modalTitle}>{CHECKLIST_TITULO_POR_TIPO[checklistActivo.tipo||'entrega']}</div><div className={styles.modalSub}>{checklistActivo.vehiculo} · {checklistActivo.patente} · {checklistActivo.fecha_entrega?new Date(checklistActivo.fecha_entrega+'T12:00:00').toLocaleDateString('es-AR'):''}</div><div style={{marginTop:'1rem'}}><div style={{fontSize:'13px',marginBottom:'12px'}}><span style={{color:'#9AA3B8'}}>Mecánico:</span> <b>{checklistActivo.mecanico||'—'}</b></div>{checklistActivo.fotos&&checklistActivo.fotos.length>0&&<div style={{marginBottom:'12px'}}><div style={{fontSize:'11px',fontWeight:'700',color:'#9AA3B8',textTransform:'uppercase',marginBottom:'6px'}}>Fotos</div><div style={{display:'flex',gap:'6px',flexWrap:'wrap'}}>{checklistActivo.fotos.map((f,i)=><img key={f.id||i} src={f.url} alt="foto" onClick={()=>setFotoZoom(f.url)} style={{width:'56px',height:'56px',objectFit:'cover',borderRadius:'8px',border:'1px solid #343B4B',cursor:'zoom-in'}}/>)}</div></div>}<table className={styles.table}><thead><tr><th>Ítem</th><th style={{textAlign:'center'}}>Sí</th><th style={{textAlign:'center'}}>No</th><th>Observaciones</th></tr></thead><tbody>{CHECKLIST_ITEMS_POR_TIPO[checklistActivo.tipo||'entrega'].map(item=>{const v=(checklistActivo.items||{})[item]||{};return<tr key={item}><td style={{fontSize:'12px',fontWeight:'500'}}>{item}</td><td style={{textAlign:'center',fontSize:'16px',color:'#2ECC71'}}>{v.valor==='si'?'✓':''}</td><td style={{textAlign:'center',fontSize:'16px',color:'#FF4D4F'}}>{v.valor==='no'?'✓':''}</td><td style={{fontSize:'12px',color:'#9AA3B8'}}>{v.obs||'—'}</td></tr>})}</tbody></table>{checklistActivo.observacion_general&&<div style={{marginTop:'8px',padding:'10px',background:'#1C2130',borderRadius:'6px',fontSize:'13px'}}><b>Obs. general:</b> {checklistActivo.observacion_general}</div>}</div><div className={styles.modalActions}>{admin&&<button className={styles.btnDanger} onClick={()=>borrarChecklist(checklistActivo.id)}><TrashIcon/></button>}<button className={styles.btnPrimary} onClick={()=>abrirEditarChecklist(checklistActivo)}>Editar</button><button className={styles.btn} onClick={()=>imprimirChecklist(checklistActivo)}>Imprimir</button><button className={styles.btn} onClick={()=>setChecklistActivo(null)}>Cerrar</button></div></div></div>}

      {confirmDialog&&<div className={styles.modalOverlay}><div className={styles.modal}><div className={styles.modalTitle}>Confirmar</div><div style={{marginTop:'1rem',fontSize:'14px',color:'#EEF1F7',lineHeight:1.5}}>{confirmDialog.mensaje}</div><div className={styles.modalActions}><button className={styles.btn} onClick={()=>setConfirmDialog(null)}>Cancelar</button><button className={styles.btnDangerSolid} onClick={()=>{confirmDialog.onConfirm();setConfirmDialog(null)}}>Confirmar</button></div></div></div>}
      ...
      <div className={`${styles.sidebar} ${sidebarOpen?styles.sidebarOpen:''}`}>
        ...
        <div style={{padding:'0 12px 10px',position:'relative'}}>
          <input
            ...
            style={{width:'100%',padding:'8px 10px',borderRadius:'6px',border:'1px solid #2D3748',background:'#0F1117',color:'#EEF1F7',fontSize:'12.5px',fontFamily:'inherit',outline:'none',boxSizing:'border-box'}}
          />
          {busquedaGlobal.trim().length>=2&&(
            <div style={{position:'absolute',top:'100%',left:'12px',right:'12px',background:'#1A1A2E',border:'1px solid #2D3748',borderRadius:'8px',zIndex:200,maxHeight:'340px',overflowY:'auto',boxShadow:'0 8px 24px rgba(0,0,0,.4)'}}>
              {!hayResultadosGlobales&&<div style={{padding:'12px',fontSize:'12px',color:'#9AA3B8'}}>Sin resultados</div>}
              {resultadosGlobales.clientes.length>0&&<div style={{padding:'8px 10px 4px',fontSize:'10px',color:'#9AA3B8',textTransform:'uppercase',letterSpacing:'.05em'}}>Clientes</div>}
              {resultadosGlobales.clientes.map(t=>(
                <div key={t.id} onClick={()=>{verDetalle(t);setBusquedaGlobal('');setSidebarOpen(false)}} style={{padding:'8px 10px',fontSize:'12.5px',color:'#EEF1F7',cursor:'pointer',borderTop:'1px solid #24283a'}} onMouseOver={e=>e.currentTarget.style.background='#24283a'} onMouseOut={e=>e.currentTarget.style.background='transparent'}>
                  <b>{t.vehiculos?.clientes?.nombre}</b> · {t.vehiculos?.marca_modelo}
                </div>
              ))}
              {resultadosGlobales.presupuestos.length>0&&<div style={{padding:'8px 10px 4px',fontSize:'10px',color:'#9AA3B8',textTransform:'uppercase',letterSpacing:'.05em'}}>Presupuestos</div>}
              {resultadosGlobales.presupuestos.map(p=>(
                <div key={p.id} onClick={()=>{abrirPresupuesto(p);setSeccion('presupuesto');setBusquedaGlobal('');setSidebarOpen(false)}} style={{padding:'8px 10px',fontSize:'12.5px',color:'#EEF1F7',cursor:'pointer',borderTop:'1px solid #24283a'}} onMouseOver={e=>e.currentTarget.style.background='#24283a'} onMouseOut={e=>e.currentTarget.style.background='transparent'}>
                  <span style={{fontFamily:'monospace',color:'#94A3B8'}}>{p.numero}</span> · {p.cliente||'—'}
                </div>
              ))}
              {resultadosGlobales.checklists.length>0&&<div style={{padding:'8px 10px 4px',fontSize:'10px',color:'#9AA3B8',textTransform:'uppercase',letterSpacing:'.05em'}}>Checklists</div>}
              {resultadosGlobales.checklists.map(c=>(
                <div key={c.id} onClick={()=>{setChecklistActivo(c);setBusquedaGlobal('');setSidebarOpen(false)}} style={{padding:'8px 10px',fontSize:'12.5px',color:'#EEF1F7',cursor:'pointer',borderTop:'1px solid #24283a'}} onMouseOver={e=>e.currentTarget.style.background='#24283a'} onMouseOut={e=>e.currentTarget.style.background='transparent'}>
      ...
      <div className={styles.main}>
          ...
          <div>
            <div className={styles.topBar}>
              ...
              <div style={{display:'flex',gap:'8px',alignItems:'center',flexWrap:'wrap'}}>
                ...
                <span style={{fontSize:'13px',fontWeight:'600',color:'#EEF1F7',minWidth:'150px',textAlign:'center'}}>{new Date(fechaPlan+'T12:00:00').toLocaleDateString('es-AR',{weekday:'long',day:'numeric',month:'long'})}</span>
                <button className={styles.btn} onClick={()=>cambiarDiaPlan(1)}>›</button>
                ...
                <button className={styles.btnSuccess} onClick={compartirPlanDiario}>Compartir</button>
                <button className={styles.btn} onClick={imprimirPlanDiario}>Imprimir</button>
                {admin&&<button className={styles.btnPrimary} onClick={()=>setModalAgregarPlan('elegir')}>+ Agregar</button>}
              </div>
            </div>
            <div className={styles.divider}></div>

            {planDiario.length===0&&<div style={{color:'#6B7488',fontSize:'14px',textAlign:'center',padding:'3rem'}}>Sin tareas planificadas para este día</div>}

            {planPorMecanico.map(([mecanico,tareas])=>(
              <div key={mecanico} className={styles.card}>
                <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'12px'}}>
                  <div style={{width:'26px',height:'26px',borderRadius:'50%',background:'rgba(47,168,255,.12)',color:'#2FA8FF',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'11px',fontWeight:'700'}}>{mecanico==='Sin asignar'?'?':mecanico.split(' ').map(w=>w[0]).slice(0,2).join('').toUpperCase()}</div>
                  <div className={styles.cardTitle} style={{margin:0}}>{mecanico} — {tareas.length} {tareas.length===1?'tarea':'tareas'}</div>
                </div>
                {tareas.map(p=>{
                  const trabajoRef=p.tipo==='vehiculo'?trabajos.find(t=>t.id===p.trabajo_id):null
                  return (
                    <div key={p.id} style={{display:'flex',gap:'10px',alignItems:'flex-start',padding:'8px 0',borderBottom:'1px solid #262B38'}}>
                      ...
                      <div style={{flex:1,opacity:p.completado?0.5:1}}>
                        {p.tipo==='vehiculo'?(
                          <>
                            ...
                            {p.descripcion&&<div style={{fontSize:'12.5px',color:'#9AA3B8',marginTop:'2px',textDecoration:p.completado?'line-through':'none'}}>{p.descripcion}</div>}
                          </>
                        ):(
                          <div style={{fontSize:'13.5px',textDecoration:p.completado?'line-through':'none'}}>{p.descripcion}</div>
                        )}
                      </div>
                      {admin&&<button className={styles.btnDelete} style={{fontSize:'10px',padding:'3px 7px'}} onClick={()=>borrarTareaPlan(p)}><TrashIcon/></button>}
                    </div>
                  )
                })}
              </div>
        ...
        {seccion==='dashboard'&&!tallerVista&&!vistaStats&&!vistaMarca&&(
          <div>
            ...
            <div className={styles.divider}></div>
            <div className={styles.stats}><div className={styles.stat} style={{cursor:'default'}}><div className={styles.statN}>{stats.total}</div><div className={styles.statL}>Clientes totales</div></div><div className={styles.stat} style={{cursor:'pointer'}} onClick={()=>setVistaStats('enTaller')}><div className={styles.statN}>{stats.enTaller}</div><div className={styles.statL}>En taller →</div></div><div className={styles.stat} style={{cursor:'pointer'}} onClick={()=>setVistaStats('listos')}><div className={styles.statN}>{stats.listos}</div><div className={styles.statL}>Listos →</div></div><div className={styles.stat} style={{cursor:'pointer'}} onClick={()=>setVistaStats('salidos')}><div className={styles.statN}>{stats.salidos}</div><div className={styles.statL}>Entregados →</div></div><div className={styles.stat} style={{cursor:'pointer'}} onClick={()=>setVistaStats('reingresos')}><div className={styles.statN}>{reingresosRaw.length}</div><div className={styles.statL}>Reingresos →</div></div><div className={styles.stat} style={{cursor:'pointer'}} onClick={()=>setVistaStats('estancados')}><div className={styles.statN} style={{color:trabajosEstancados.length>0?'#FF4D4F':undefined}}>{trabajosEstancados.length}</div><div className={styles.statL}>Estancados →</div></div></div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'1rem',marginBottom:'1rem'}}>{['Malvinas 2084','Malvinas 3906'].map(taller=>{const t=trabajos.filter(x=>x.taller===taller&&x.estado!=='Salio');return<div key={taller} className={styles.tallerCard} onClick={()=>setTallerVista(taller)}><div className={styles.tallerNombre}>{taller}</div><div className={styles.tallerN}>{t.length} <span>autos</span></div><div style={{marginTop:'10px'}}>{['Diagnóstico','En proceso','En espera','Desarmando','Listo'].map(estado=>{const n=t.filter(x=>x.estado===estado).length;return n>0?<div key={estado} style={{display:'flex',justifyContent:'space-between',fontSize:'12px',padding:'4px 0',borderBottom:'1px solid #262B38'}}><span style={{color:'#9AA3B8'}}>{estado}</span><span style={{color:'#EEF1F7',fontWeight:'600'}}>{n}</span></div>:null})}</div><div className={styles.tallerBtn}>Ver detalle →</div></div>})}</div>
            <div className={styles.card}><div className={styles.cardTitle}>Marcas en taller</div><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(130px,1fr))',gap:'8px'}}>{Object.entries(conteoMarcas).sort((a,b)=>b[1]-a[1]).map(([marca,n])=>(<div key={marca} onClick={()=>setVistaMarca(marca)} style={{background:'#1C2130',borderRadius:'8px',padding:'10px 14px',display:'flex',justifyContent:'space-between',alignItems:'center',border:'1px solid #343B4B',cursor:'pointer'}} onMouseOver={e=>{e.currentTarget.style.borderColor='#2FA8FF';e.currentTarget.style.background='#20263A'}} onMouseOut={e=>{e.currentTarget.style.borderColor='#343B4B';e.currentTarget.style.background='#1C2130'}}><span style={{fontSize:'13px',color:'#9AA3B8',fontWeight:'500'}}>{marca}</span><span style={{fontSize:'20px',fontWeight:'700',color:'#2FA8FF'}}>{n}</span></div>))}{Object.keys(conteoMarcas).length===0&&<div style={{color:'#6B7488',fontSize:'13px'}}>Sin autos en taller</div>}</div></div>
          </div>
        )}

        {seccion==='dashboard'&&!tallerVista&&!vistaStats&&vistaMarca&&(<div><div className={styles.topBar}><button className={styles.btn} onClick={()=>setVistaMarca(null)}>← Volver</button><h1 className={styles.pageTitle}>{vistaMarca} en taller</h1></div><div className={styles.divider}></div><div className={styles.tblWrap}><table className={styles.table}><thead><tr><th>#</th><th>Vehículo</th><th>Cliente</th><th>Patente</th><th>Estado</th><th>Taller</th><th>Ingreso</th></tr></thead><tbody>{trabajosDeMarca.map((t,i)=><tr key={t.id} onClick={()=>verDetalle(t)}><td style={{color:'#6B7488'}}>{i+1}</td><td><b>{t.vehiculos?.marca_modelo}</b></td><td>{t.vehiculos?.clientes?.nombre}</td><td>{t.vehiculos?.patente}</td><td><span className={badgeClass(t.estado)}>{t.estado}</span></td><td>{t.taller}</td><td style={{fontSize:'12px',color:'#9AA3B8'}}>{new Date(t.fecha_ingreso).toLocaleDateString('es-AR')}</td></tr>)}{trabajosDeMarca.length===0&&<tr><td colSpan="7" style={{textAlign:'center',color:'#6B7488',padding:'2rem'}}>Sin resultados</td></tr>}</tbody></table></div></div>)}

        {seccion==='dashboard'&&!tallerVista&&vistaStats&&vistaStats!=='reingresos'&&vistaStats!=='estancados'&&(<div><div className={styles.topBar}><button className={styles.btn} onClick={()=>setVistaStats(null)}>← Volver</button><h1 className={styles.pageTitle}>{titulosVistaStats[vistaStats]}</h1></div><div className={styles.divider}></div><div className={styles.tblWrap}><table className={styles.table}><thead><tr><th>#</th><th>Vehículo</th><th>Cliente</th><th>Patente</th><th>Estado</th><th>Taller</th><th>Ingreso</th></tr></thead><tbody>{listaVistaStats[vistaStats].map((t,i)=><tr key={t.id} onClick={()=>verDetalle(t)}><td style={{color:'#6B7488'}}>{i+1}</td><td><b>{t.vehiculos?.marca_modelo}</b></td><td>{t.vehiculos?.clientes?.nombre}</td><td>{t.vehiculos?.patente}</td><td><span className={badgeClass(t.estado)}>{t.estado}</span></td><td>{t.taller}</td><td style={{fontSize:'12px',color:'#9AA3B8'}}>{new Date(t.fecha_ingreso).toLocaleDateString('es-AR')}</td></tr>)}{listaVistaStats[vistaStats].length===0&&<tr><td colSpan="7" style={{textAlign:'center',color:'#6B7488',padding:'2rem'}}>Sin resultados</td></tr>}</tbody></table></div></div>)}

        {seccion==='dashboard'&&!tallerVista&&vistaStats==='estancados'&&(<div><div className={styles.topBar}><button className={styles.btn} onClick={()=>setVistaStats(null)}>← Volver</button><h1 className={styles.pageTitle}>Vehículos estancados (+{umbralEstancados} días sin cambiar de estado)</h1>{!editandoUmbral?<button className={styles.btn} onClick={()=>{setUmbralTemp(String(umbralEstancados));setEditandoUmbral(true)}}>Cambiar umbral</button>:<div style={{display:'flex',gap:'6px',alignItems:'center'}}><input type="number" min="1" value={umbralTemp} onChange={e=>setUmbralTemp(e.target.value)} style={{width:'60px',padding:'6px 8px',borderRadius:'6px',border:'1px solid #343B4B',background:'#171B25',color:'#EEF1F7',fontSize:'13px',fontFamily:'inherit'}}/><span style={{fontSize:'12px',color:'#9AA3B8'}}>días</span><button className={styles.btnPrimary} style={{fontSize:'12px',padding:'6px 12px'}} onClick={guardarUmbralEstancados}>Guardar</button><button className={styles.btn} style={{fontSize:'12px',padding:'6px 12px'}} onClick={()=>setEditandoUmbral(false)}>Cancelar</button></div>}</div><div className={styles.divider}></div><div className={styles.tblWrap}><table className={styles.table}><thead><tr><th>#</th><th>Vehículo</th><th>Cliente</th><th>Patente</th><th>Estado</th><th>Días</th><th></th></tr></thead><tbody>{trabajosEstancados.map((t,i)=><tr key={t.id}><td style={{color:'#6B7488'}}>{i+1}</td><td onClick={()=>verDetalle(t)}><b>{t.vehiculos?.marca_modelo}</b></td><td onClick={()=>verDetalle(t)}>{t.vehiculos?.clientes?.nombre}</td><td onClick={()=>verDetalle(t)}>{t.vehiculos?.patente}</td><td onClick={()=>verDetalle(t)}><span className={badgeClass(t.estado)}>{t.estado}</span></td><td onClick={()=>verDetalle(t)} style={{fontWeight:'700',color:'#FF4D4F'}}>{diasDesde(fechaReferenciaEstancado(t))} días</td><td style={{cursor:'default'}}><button className={styles.btn} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>toggleExcluirEstancados(t)}>Excluir</button></td></tr>)}{trabajosEstancados.length===0&&<tr><td colSpan="7" style={{textAlign:'center',color:'#6B7488',padding:'2rem'}}>Ningún vehículo estancado</td></tr>}</tbody></table></div>
        {estancadosExcluidos.length>0&&<div className={styles.card} style={{marginTop:'1rem'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',cursor:'pointer'}} onClick={()=>setMostrarExcluidosEstancados(v=>!v)}>
            ...
            <span style={{fontSize:'12px',color:'#9AA3B8'}}>{mostrarExcluidosEstancados?'▲ Ocultar':'▼ Ver'}</span>
          </div>
          {mostrarExcluidosEstancados&&<>
            <div style={{fontSize:'12px',color:'#6B7488',margin:'10px 0'}}>Estos vehículos llevan más de {umbralEstancados} días sin cambiar de estado, pero elegiste no contarlos.</div>
            <table className={styles.table}><thead><tr><th>Vehículo</th><th>Cliente</th><th>Días</th><th></th></tr></thead><tbody>{estancadosExcluidos.map(t=>(<tr key={t.id}><td onClick={()=>verDetalle(t)}><b>{t.vehiculos?.marca_modelo}</b></td><td onClick={()=>verDetalle(t)}>{t.vehiculos?.clientes?.nombre}</td><td onClick={()=>verDetalle(t)} style={{color:'#9AA3B8'}}>{diasDesde(fechaReferenciaEstancado(t))} días</td><td style={{cursor:'default'}}><button className={styles.btn} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>toggleExcluirEstancados(t)}>Volver a incluir</button></td></tr>))}</tbody></table>
          </>}
        </div>}
        </div>)}

        {seccion==='dashboard'&&!tallerVista&&vistaStats==='reingresos'&&(<div><div className={styles.topBar}><button className={styles.btn} onClick={()=>setVistaStats(null)}>← Volver</button><h1 className={styles.pageTitle}>Reingresos</h1></div><div className={styles.divider}></div><div className={styles.tblWrap}><table className={styles.table}><thead><tr><th>#</th><th>Vehículo</th><th>Cliente</th><th>Patente</th><th>Veces que reingresó</th></tr></thead><tbody>{reingresosPorVehiculo.map((r,i)=><tr key={r.vehiculo?.id||i} onClick={()=>{const t=trabajos.find(tr=>tr.vehiculos?.id===r.vehiculo?.id);if(t)verDetalle(t)}}><td style={{color:'#6B7488'}}>{i+1}</td><td><b>{r.vehiculo?.marca_modelo}</b></td><td>{r.vehiculo?.clientes?.nombre}</td><td>{r.vehiculo?.patente}</td><td style={{fontWeight:'700',color:'#2FA8FF'}}>{r.cantidad} {r.cantidad===1?'vez':'veces'}</td></tr>)}{reingresosPorVehiculo.length===0&&<tr><td colSpan="5" style={{textAlign:'center',color:'#6B7488',padding:'2rem'}}>Sin reingresos registrados</td></tr>}</tbody></table></div></div>)}

        {seccion==='dashboard'&&tallerVista&&(<div><div className={styles.topBar}><button className={styles.btn} onClick={()=>setTallerVista(null)}>← Volver</button><h1 className={styles.pageTitle}>{tallerVista}</h1></div><div className={styles.divider}></div><div className={styles.tblWrap}><table className={styles.table}><thead><tr><th>#</th><th>Vehículo</th><th>Cliente</th><th>Patente</th><th>Estado</th><th>Mecánico</th><th>Ingreso</th>{admin&&<th>Acciones</th>}</tr></thead><tbody>{trabajosTaller.map((t,i)=>(<tr key={t.id}><td style={{color:'#6B7488'}}>{i+1}</td><td onClick={()=>verDetalle(t)}><b>{t.vehiculos?.marca_modelo}</b></td><td onClick={()=>verDetalle(t)}>{t.vehiculos?.clientes?.nombre}</td><td onClick={()=>verDetalle(t)}>{t.vehiculos?.patente}</td><td onClick={()=>verDetalle(t)}><span className={badgeClass(t.estado)}>{t.estado}</span></td><td onClick={()=>verDetalle(t)}>{t.mecanico||'—'}</td><td onClick={()=>verDetalle(t)} style={{fontSize:'12px',color:'#9AA3B8'}}>{new Date(t.fecha_ingreso).toLocaleDateString('es-AR')}</td>{admin&&<td style={{display:'flex',gap:'5px',cursor:'default'}}><button className={styles.btnSuccess} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>{setModalActualizar(t);setFormActualizar({tipo:'estado',descripcion:'',taller_nuevo:'Malvinas 3906',mecanico:'',fecha_manual:''})}}>Actualizar</button>{t.estado!=='Salio'&&<button className={styles.btnDangerSolid} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>setModalSalida(t)}>Salida</button>}<button className={styles.btnEdit} onClick={()=>abrirEditar(t)}>Editar</button></td>}</tr>))}</tbody></table></div></div>)}

        {seccion==='clientes'&&!verEntregados&&!verPapelera&&(<div><div className={styles.topBar}><h1 className={styles.pageTitle}>Clientes</h1><div style={{display:'flex',gap:'8px',flexWrap:'wrap'}}><button className={styles.btn} onClick={actualizarTodo}>↻ Actualizar</button>{admin&&trabajosBorrados.length>0&&<button className={styles.btn} onClick={()=>setVerPapelera(true)}>Papelera ({trabajosBorrados.length})</button>}{admin&&<button className={styles.btnDangerSolid} onClick={()=>setVerEntregados(true)}>Vehículos entregados ({trabajosEntregados.length})</button>}{admin&&<button className={styles.btnPrimary} onClick={()=>setSeccion('nuevo')}>+ Nuevo cliente</button>}</div></div><div className={styles.divider}></div><div className={styles.searchBar}><input type="text" placeholder="Buscar por nombre, patente o vehículo..." value={busqueda} onChange={e=>{setBusqueda(e.target.value);setLimiteClientes(40)}}/></div><div className={styles.tblWrap}>{loading?<p className={styles.loading}>Cargando...</p>:(<table className={styles.table}><thead><tr><th>#</th><th>Vehículo</th><th>Cliente</th><th>Patente</th><th>Estado</th><th>Taller</th><th>Ingreso</th>{admin&&<th>Acciones</th>}</tr></thead><tbody>{trabajosFiltrados.slice(0,limiteClientes).map((t,i)=>(<tr key={t.id}><td style={{color:'#6B7488',width:'40px'}}>{totalFiltrados-i}</td><td onClick={()=>verDetalle(t)}><b>{t.vehiculos?.marca_modelo}</b></td><td onClick={()=>verDetalle(t)}>{t.vehiculos?.clientes?.nombre}</td><td onClick={()=>verDetalle(t)}>{t.vehiculos?.patente}</td><td onClick={()=>verDetalle(t)}><span className={badgeClass(t.estado)}>{t.estado}</span>{diasDesde(fechaReferenciaEstancado(t))>=umbralEstancados&&!t.excluir_estancados&&<span style={{marginLeft:'6px',fontSize:'10px',fontWeight:'700',color:'#FF4D4F',background:'rgba(255,77,79,.15)',padding:'2px 6px',borderRadius:'10px'}}>{diasDesde(fechaReferenciaEstancado(t))}d</span>}</td><td onClick={()=>verDetalle(t)}>{t.taller}</td><td onClick={()=>verDetalle(t)} style={{fontSize:'12px',color:'#9AA3B8'}}>{new Date(t.fecha_ingreso).toLocaleDateString('es-AR')}</td>{admin&&<td style={{display:'flex',gap:'5px',cursor:'default',flexWrap:'wrap'}}><button className={styles.btnSuccess} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>{setModalActualizar(t);setFormActualizar({tipo:'estado',descripcion:'',taller_nuevo:'Malvinas 3906',mecanico:'',fecha_manual:''})}}>Actualizar</button>{t.estado!=='Salio'&&<button className={styles.btnDangerSolid} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>setModalSalida(t)}>Salida</button>}<button className={styles.btnEdit} onClick={()=>abrirEditar(t)}>Editar</button><button className={styles.btnDelete} onClick={()=>borrarCliente(t)}><TrashIcon/></button></td>}</tr>))}</tbody></table>)}{trabajosFiltrados.length>limiteClientes&&<button className={styles.btn} style={{width:'100%',marginTop:'10px'}} onClick={()=>setLimiteClientes(l=>l+40)}>Ver más ({trabajosFiltrados.length-limiteClientes} restantes)</button>}</div></div>)}

        {seccion==='clientes'&&verPapelera&&admin&&(<div><div className={styles.topBar}><button className={styles.btn} onClick={()=>setVerPapelera(false)}>← Volver</button><h1 className={styles.pageTitle}>Papelera ({trabajosBorrados.length})</h1></div><div className={styles.divider}></div><div className={styles.tblWrap}><table className={styles.table}><thead><tr><th>#</th><th>Vehículo</th><th>Cliente</th><th>Patente</th><th>Taller</th><th>Acciones</th></tr></thead><tbody>{trabajosBorrados.map((t,i)=>(<tr key={t.id}><td style={{color:'#6B7488'}}>{i+1}</td><td><b>{t.vehiculos?.marca_modelo}</b></td><td>{t.vehiculos?.clientes?.nombre}</td><td>{t.vehiculos?.patente}</td><td>{t.taller}</td><td style={{display:'flex',gap:'5px',cursor:'default'}}><button className={styles.btnSuccess} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>restaurarCliente(t)}>Restaurar</button><button className={styles.btnDelete} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>borrarClienteDefinitivo(t)}><TrashIcon/> Definitivo</button></td></tr>))}{trabajosBorrados.length===0&&<tr><td colSpan="6" style={{textAlign:'center',color:'#6B7488',padding:'2rem'}}>La papelera está vacía</td></tr>}</tbody></table></div></div>)}

        {seccion==='clientes'&&verEntregados&&admin&&(<div><div className={styles.topBar}><button className={styles.btn} onClick={()=>setVerEntregados(false)}>← Volver</button><h1 className={styles.pageTitle}>Vehículos entregados ({trabajosEntregados.length})</h1></div><div className={styles.divider}></div><div className={styles.searchBar} style={{marginBottom:'12px'}}><input type="text" placeholder="Buscar por nombre, patente o vehículo..." value={busquedaEntregados} onChange={e=>setBusquedaEntregados(e.target.value)}/></div><div className={styles.tblWrap}><table className={styles.table}><thead><tr><th>#</th><th>Vehículo</th><th>Cliente</th><th>Patente</th><th>Taller</th><th>Entregado</th><th>Acciones</th></tr></thead><tbody>{trabajosEntregadosFiltrados.map((t,i)=>(<tr key={t.id}><td style={{color:'#6B7488'}}>{i+1}</td><td onClick={()=>verDetalle(t)}><b>{t.vehiculos?.marca_modelo}</b></td><td onClick={()=>verDetalle(t)}>{t.vehiculos?.clientes?.nombre}</td><td onClick={()=>verDetalle(t)}>{t.vehiculos?.patente}</td><td onClick={()=>verDetalle(t)}>{t.taller}</td><td onClick={()=>verDetalle(t)} style={{fontSize:'12px',color:'#9AA3B8'}}>{t.fecha_salida?new Date(t.fecha_salida).toLocaleDateString('es-AR'):'—'}</td><td style={{display:'flex',gap:'5px',cursor:'default'}}><button className={styles.btnPrimary} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>{setModalReingreso(t);setFormReingreso({motivo:'',mecanico:t.mecanico||'',taller:t.taller||'Malvinas 2084',estado:'Diagnóstico',llego_en_grua:false,fecha_ingreso_manual:''})}}>Reingreso</button><button style={{fontSize:'11px',padding:'4px 8px',background:'rgba(46,204,113,.12)',color:'#2ECC71',border:'1px solid rgba(46,204,113,.4)',borderRadius:'6px',cursor:'pointer',fontFamily:"'Barlow Condensed',sans-serif",fontWeight:'700',textTransform:'uppercase',letterSpacing:'.03em'}} onClick={()=>abrirWsp(t)}>WSP</button><button className={styles.btnEdit} onClick={()=>abrirEditar(t)}>Editar</button><button className={styles.btnDelete} onClick={()=>borrarCliente(t)}><TrashIcon/></button></td></tr>))}{trabajosEntregadosFiltrados.length===0&&<tr><td colSpan="7" style={{textAlign:'center',color:'#6B7488',padding:'2rem'}}>{busquedaEntregados?'Sin resultados':'Sin vehículos entregados todavía'}</td></tr>}</tbody></table></div></div>)}
        ...
        {seccion==='turnos'&&admin&&(
          <div>
            ...
            {verRecordatorios&&(<div className={styles.card} style={{marginBottom:'1rem'}}>
              ...
              {turnosManiana.length===0&&<div style={{color:'#6B7488',fontSize:'13px',textAlign:'center',padding:'1rem'}}>No hay turnos para mañana</div>}
              {turnosManiana.map(t=>(
                <div key={t.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'10px 12px',background:'#1C2130',borderRadius:'8px',border:'1px solid #343B4B',marginBottom:'8px'}}>
                  <div><div style={{fontSize:'13px',fontWeight:'600',color:'#EEF1F7'}}>{t.nombre}</div><div style={{fontSize:'12px',color:'#9AA3B8'}}>{t.vehiculo||'Sin vehículo'} · 8:30 hs</div></div>
                  {t.telefono?<button style={{fontSize:'12px',padding:'6px 12px',background:'rgba(46,204,113,.12)',color:'#2ECC71',border:'1px solid rgba(46,204,113,.4)',borderRadius:'6px',cursor:'pointer',fontFamily:"'Barlow Condensed',sans-serif",fontWeight:'700',textTransform:'uppercase',letterSpacing:'.03em'}} onClick={()=>enviarRecordatorioTurno(t)}>Enviar</button>:<span style={{fontSize:'11px',color:'#6B7488'}}>Sin teléfono</span>}
                </div>
              ))}
            </div>)}
            <div className={styles.divider}></div>
                ...
                <div style={{display:'flex',flexDirection:'column',gap:'1rem'}}>
                  <div className={styles.card}>
                    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'12px'}}>
                      ...
                      <div style={{fontSize:'11px',color:turnosPorDia(diaSeleccionado).length>=MAX_TURNOS_POR_DIA?'#FF4D4F':'#2ECC71',fontWeight:'600',background:turnosPorDia(diaSeleccionado).length>=MAX_TURNOS_POR_DIA?'rgba(255,77,79,.15)':'rgba(46,204,113,.15)',padding:'2px 8px',borderRadius:'20px'}}>{turnosPorDia(diaSeleccionado).length}/{MAX_TURNOS_POR_DIA}</div>
                    </div>
                    {turnosPorDia(diaSeleccionado).length===0&&<div style={{color:'#6B7488',fontSize:'13px',textAlign:'center',padding:'1rem'}}>Sin turnos este día</div>}
                    {turnosPorDia(diaSeleccionado).map(t=>{
                      const estadoTurno=t.estado||'pendiente'
                      const estadoInfo={
                        pendiente:{label:'Pendiente',bg:'#262B38',color:'#9AA3B8'},
                        presento:{label:'Se presentó',bg:'rgba(46,204,113,.15)',color:'#2ECC71'},
                        no_presento:{label:'No se presentó',bg:'rgba(255,77,79,.15)',color:'#FF4D4F'},
                        reprogramado:{label:'Reprogramado',bg:'rgba(245,183,0,.18)',color:'#8A6100'}
                      }[estadoTurno]
                      return(
                      <div key={t.id} style={{padding:'10px 12px',background:'#1C2130',borderRadius:'8px',border:'1px solid #343B4B',marginBottom:'8px'}}>
                        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                          <div><div style={{fontSize:'13px',fontWeight:'600',color:'#EEF1F7'}}>{t.nombre}</div><div style={{fontSize:'12px',color:'#9AA3B8'}}>{t.vehiculo||'Sin vehículo'}</div>{t.motivo&&<div style={{fontSize:'11px',color:'#6B7488',marginTop:'2px'}}>{t.motivo}</div>}</div>
                          <div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:'4px'}}>
                            <div style={{fontSize:'11px',fontWeight:'700',color:'#2FA8FF'}}>8:30 hs</div>
                        ...
                        <div style={{display:'flex',gap:'4px',marginTop:'8px',flexWrap:'wrap'}}>
                          {t.telefono&&<button style={{fontSize:'11px',padding:'3px 8px',background:'rgba(46,204,113,.12)',color:'#2ECC71',border:'1px solid rgba(46,204,113,.4)',borderRadius:'6px',cursor:'pointer',fontFamily:"'Barlow Condensed',sans-serif",fontWeight:'700',textTransform:'uppercase',letterSpacing:'.03em'}} onClick={()=>{let tel=t.telefono.replace(/\D/g,'');if(!tel.startsWith('54'))tel='54'+tel;const fechaF=new Date(t.fecha+'T12:00:00').toLocaleDateString('es-AR',{weekday:'long',year:'numeric',month:'long',day:'numeric'});const msg=`Hola ${t.nombre}! Te recordamos tu turno en DiFiore Performance.\n\n📅 Fecha: ${fechaF}\n🕘 Horario: 8:30 hs\n🚗 Vehículo: ${t.vehiculo||'—'}\n\nTe esperamos en Malvinas 2084, Mar del Plata.`;window.open(`https://wa.me/${tel}?text=${encodeURIComponent(msg)}`,'_blank')}}>WSP</button>}
                          <button className={styles.btnEdit} style={{fontSize:'11px',padding:'3px 8px'}} onClick={()=>abrirEditarTurno(t)}>Editar</button>
                          <button className={styles.btnDelete} style={{fontSize:'11px',padding:'3px 8px'}} onClick={()=>borrarTurno(t.id)}><TrashIcon/></button>
                        </div>
                        <div style={{display:'flex',gap:'4px',marginTop:'6px',flexWrap:'wrap',borderTop:'1px solid #343B4B',paddingTop:'6px'}}>
                          <button style={{fontSize:'10px',padding:'3px 7px',background:estadoTurno==='presento'?'#2ECC71':'#262B38',color:estadoTurno==='presento'?'#0B0D12':'#9AA3B8',border:'1px solid #343B4B',borderRadius:'6px',cursor:'pointer',fontFamily:"'Barlow Condensed',sans-serif",fontWeight:'700',textTransform:'uppercase',letterSpacing:'.03em'}} onClick={()=>usarTurnoComoNuevoCliente(t)}>Se presentó</button>
                          <button style={{fontSize:'10px',padding:'3px 7px',background:estadoTurno==='no_presento'?'#FF4D4F':'#262B38',color:estadoTurno==='no_presento'?'#fff':'#9AA3B8',border:'1px solid #343B4B',borderRadius:'6px',cursor:'pointer',fontFamily:"'Barlow Condensed',sans-serif",fontWeight:'700',textTransform:'uppercase',letterSpacing:'.03em'}} onClick={()=>marcarEstadoTurno(t,'no_presento')}>No se presentó</button>
                          <button style={{fontSize:'10px',padding:'3px 7px',background:estadoTurno==='reprogramado'?'#F5B700':'#262B38',color:estadoTurno==='reprogramado'?'#0B0D12':'#9AA3B8',border:'1px solid #343B4B',borderRadius:'6px',cursor:'pointer',fontFamily:"'Barlow Condensed',sans-serif",fontWeight:'700',textTransform:'uppercase',letterSpacing:'.03em'}} onClick={()=>{marcarEstadoTurno(t,'reprogramado');abrirEditarTurno(t)}}>Reprogramó</button>
                        </div>
                      </div>
                      )
                    })}
        ...
        {seccion==='presupuesto'&&admin&&(
          <div>
                    ...
                    <tbody>
                      {presupuestosFiltrados.map(p=>(
                        <tr key={p.id}>
                          ...
                          <td onClick={()=>abrirPresupuesto(p)} style={{fontSize:'12px',color:'#9AA3B8'}}>{p.fecha?new Date(p.fecha+'T12:00:00').toLocaleDateString('es-AR'):'—'}</td>
                          ...
                          <td style={{display:'flex',gap:'5px',cursor:'default'}}>
                            <button className={styles.btnEdit} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>abrirPresupuesto(p)}>Editar</button>
                            <button className={styles.btnDelete} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>borrarPresupuesto(p)}><TrashIcon/></button>
                          </td>
                        </tr>
                      ))}
                      {presupuestosFiltrados.length===0&&<tr><td colSpan="6" style={{textAlign:'center',color:'#6B7488',padding:'2rem'}}>Sin presupuestos guardados todavía</td></tr>}
                    </tbody>
                  </table>
                </div>
              </div>
            ...
            <div className={styles.card}>
              ...
              <div style={{marginBottom:'12px',padding:'10px 14px',background:'rgba(47,168,255,.12)',borderRadius:'8px',border:'1px solid rgba(47,168,255,.35)',display:'flex',alignItems:'center',gap:'20px'}}><span style={{fontSize:'12px',color:'#2FA8FF',fontWeight:'600'}}>Moneda mano de obra:</span><label style={{display:'flex',alignItems:'center',gap:'6px',fontSize:'13px',cursor:'pointer'}}><input type="radio" value="ARS" checked={presupuesto.moneda_mano_obra==='ARS'} onChange={e=>setPresupuesto({...presupuesto,moneda_mano_obra:e.target.value})}/> $ Pesos</label><label style={{display:'flex',alignItems:'center',gap:'6px',fontSize:'13px',cursor:'pointer'}}><input type="radio" value="USD" checked={presupuesto.moneda_mano_obra==='USD'} onChange={e=>setPresupuesto({...presupuesto,moneda_mano_obra:e.target.value})}/> USS Dólar</label>{presupuesto.moneda_mano_obra==='USD'&&<span style={{fontSize:'12px',color:'#FF4D4F',fontWeight:'600'}}>El total se mostrará en USS sin conversión a pesos</span>}</div>
              {presupuesto.items.map((item,idx)=>(<div key={idx} style={{background:'#1C2130',border:'1px solid #343B4B',borderRadius:'8px',padding:'12px',marginBottom:'10px'}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'10px'}}><div style={{display:'flex',alignItems:'center',gap:'12px'}}><span style={{fontSize:'12px',fontWeight:'600',color:'#9AA3B8'}}>Ítem {idx+1}</span><label style={{display:'flex',alignItems:'center',gap:'5px',fontSize:'12px',cursor:'pointer',color:'#2FA8FF',fontWeight:'500'}}><input type="checkbox" checked={item.es_mano_obra} onChange={e=>{const items=[...presupuesto.items];items[idx]={...items[idx],es_mano_obra:e.target.checked};setPresupuesto({...presupuesto,items})}}/> Es mano de obra</label></div>{presupuesto.items.length>1&&<button style={{background:'none',border:'none',color:'#FF4D4F',cursor:'pointer',fontSize:'18px',lineHeight:1}} onClick={()=>setPresupuesto({...presupuesto,items:presupuesto.items.filter((_,i)=>i!==idx)})}>✕</button>}</div><div className={styles.formGrid}><div className={styles.formGroup} style={{gridColumn:'1/-1'}}><label>Descripción</label><input value={item.descripcion} onChange={e=>{const items=[...presupuesto.items];items[idx]={...items[idx],descripcion:e.target.value};setPresupuesto({...presupuesto,items})}} placeholder={item.es_mano_obra?'Ej: Reparación de motor...':'Ej: Kit de distribución Alemán'}/></div><div className={styles.formGroup}><label>Precio unitario {item.es_mano_obra?`(${presupuesto.moneda_mano_obra==='USD'?'USS':'$'})`:'(opcional)'}</label><input value={item.precio_unitario} onChange={e=>{const items=[...presupuesto.items];items[idx]={...items[idx],precio_unitario:formatNum(e.target.value)};setPresupuesto({...presupuesto,items})}} placeholder="0"/></div><div className={styles.formGroup}><label>Total {item.es_mano_obra?`(${presupuesto.moneda_mano_obra==='USD'?'USS':'$'})`:'($)'}</label><input value={item.total} onChange={e=>{const items=[...presupuesto.items];items[idx]={...items[idx],total:formatNum(e.target.value)};setPresupuesto({...presupuesto,items})}} placeholder="0"/></div></div></div>))}
              {!usandoUSD&&<>
                <div style={{background:'rgba(46,204,113,.1)',border:'1px solid rgba(46,204,113,.4)',borderRadius:'8px',padding:'12px 16px',marginTop:'4px',marginBottom:'8px'}}><label style={{display:'flex',alignItems:'center',gap:'8px',fontSize:'13px',cursor:'pointer',fontWeight:'600',color:'#2ECC71',marginBottom:'10px'}}><input type="checkbox" checked={presupuesto.aplicar_descuento} onChange={e=>setPresupuesto({...presupuesto,aplicar_descuento:e.target.checked})}/>Aplicar descuento</label>{presupuesto.aplicar_descuento&&<div className={styles.formGrid}><div className={styles.formGroup}><label>Concepto del descuento</label><input value={presupuesto.descuento_concepto} onChange={e=>setPresupuesto({...presupuesto,descuento_concepto:e.target.value})} placeholder="Ej: Descuento por diagnóstico"/></div><div className={styles.formGroup}><label>Monto a descontar ($)</label><input value={presupuesto.descuento_monto} onChange={e=>setPresupuesto({...presupuesto,descuento_monto:formatNum(e.target.value)})} placeholder="100.000"/></div></div>}</div>
                <div style={{background:'rgba(47,168,255,.12)',border:'1px solid rgba(47,168,255,.35)',borderRadius:'8px',padding:'12px 16px',marginBottom:'8px'}}><label style={{display:'flex',alignItems:'center',gap:'8px',fontSize:'13px',cursor:'pointer',fontWeight:'600',color:'#2FA8FF',marginBottom:'10px'}}><input type="checkbox" checked={presupuesto.mostrar_transferencia} onChange={e=>setPresupuesto({...presupuesto,mostrar_transferencia:e.target.checked})}/>Mostrar precio transferencia / efectivo (descuento)</label>{presupuesto.mostrar_transferencia&&<div><div style={{fontSize:'12px',color:'#9AA3B8',marginBottom:'8px'}}>Aplicar recargo del 20% sobre:</div><div style={{display:'flex',gap:'16px',flexWrap:'wrap'}}><label style={{display:'flex',alignItems:'center',gap:'6px',fontSize:'13px',cursor:'pointer'}}><input type="checkbox" checked={presupuesto.transferencia_repuestos} onChange={e=>setPresupuesto({...presupuesto,transferencia_repuestos:e.target.checked})}/> Repuestos</label><label style={{display:'flex',alignItems:'center',gap:'6px',fontSize:'13px',cursor:'pointer'}}><input type="checkbox" checked={presupuesto.transferencia_mano_obra} onChange={e=>setPresupuesto({...presupuesto,transferencia_mano_obra:e.target.checked})}/> Mano de obra</label></div><div style={{marginTop:'12px',display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}><div style={{background:'#1B4CFF',color:'#fff',borderRadius:'8px',padding:'10px 14px',display:'flex',justifyContent:'space-between',alignItems:'center'}}><span style={{fontSize:'12px',fontWeight:'600'}}>Transferencia</span><span style={{fontSize:'16px',fontWeight:'900'}}>${formatPeso(Math.round(totalTransferencia))}</span></div><div style={{background:'#2ECC71',color:'#0B0D12',borderRadius:'8px',padding:'10px 14px',display:'flex',justifyContent:'space-between',alignItems:'center'}}><span style={{fontSize:'12px',fontWeight:'600'}}>Efectivo (descuento)</span><span style={{fontSize:'16px',fontWeight:'900'}}>${formatPeso(Math.round(totalEfectivo))}</span></div></div></div>}{!presupuesto.mostrar_transferencia&&<div style={{background:'#1B4CFF',color:'#fff',borderRadius:'8px',padding:'10px 14px',display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:'4px'}}><span style={{fontSize:'12px',fontWeight:'600'}}>TOTAL</span><span style={{fontSize:'16px',fontWeight:'900'}}>${formatPeso(Math.round(totalEfectivo))}</span></div>}</div>
              </>}
              {usandoUSD&&<div style={{background:'#1B4CFF',color:'#fff',borderRadius:'8px',padding:'12px 16px',marginTop:'4px',display:'flex',justifyContent:'space-between',alignItems:'center'}}><span style={{fontSize:'13px',fontWeight:'600'}}>TOTAL MANO DE OBRA</span><span style={{fontSize:'18px',fontWeight:'900'}}>USS {formatPeso(totalManoObraUSD)}</span></div>}
            </div>
...
{seccion==='recibo'&&admin&&(
        ...
        {seccion==='checklist'&&(
          <div>
            ...
            {!admin&&empleadoActual&&<div style={{background:'rgba(47,168,255,.12)',border:'1px solid rgba(47,168,255,.35)',borderRadius:'8px',padding:'8px 14px',marginBottom:'12px',fontSize:'13px',display:'flex',justifyContent:'space-between',alignItems:'center'}}><span>Mostrando registros de <b>{empleadoActual}</b></span><button className={styles.btn} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>setEmpleadoActual('')}>Cambiar</button></div>}
            {vistaChecklist==='procedimiento'&&(
              <div className={styles.card}>
                <div className={styles.cardTitle}>Procedimiento de reparación de inyectores</div>
                <div style={{background:'rgba(245,183,0,.12)',border:'1px solid rgba(245,183,0,.4)',borderRadius:'8px',padding:'12px 14px',marginBottom:'14px',fontSize:'13px',color:'#F5B700',fontWeight:'600'}}>Cuando hay trabajos de inyectores, los hace una sola persona por completo.</div>
                <div style={{display:'flex',flexDirection:'column',gap:'0'}}>
                  {PROCEDIMIENTO_INYECTORES.map((paso,idx)=>(
                    <div key={idx} style={{display:'flex',gap:'14px',padding:'14px 0',borderBottom:idx<PROCEDIMIENTO_INYECTORES.length-1?'1px solid #262B38':'none'}}>
                      <div style={{width:'28px',height:'28px',borderRadius:'50%',background:'rgba(47,168,255,.12)',color:'#2FA8FF',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'13px',fontWeight:'700',flexShrink:0}}>{idx+1}</div>
                      <div>
                        <div style={{fontSize:'14px',fontWeight:'600',color:'#EEF1F7',marginBottom:'2px'}}>{paso.titulo}</div>
                        <div style={{fontSize:'13px',color:'#9AA3B8',lineHeight:1.5}}>{paso.descripcion}</div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div>
                <div className={styles.card}>
                  ...
                  <div className={styles.formGrid}>
                    ...
                    <div className={styles.formGroup}><label>Patente</label><input value={formChecklist.patente} {...patenteHandlers(formChecklist.patente,v=>setFormChecklist({...formChecklist,patente:v}))} placeholder="AB 123 CD"/><div style={{fontSize:'11px',color:'#6B7488',marginTop:'4px'}}>No hace falta poner el espacio, se agrega solo</div></div>
                ...
                <div className={styles.card}>
                  ...
                  <div style={{display:'flex',flexDirection:'column',gap:'0'}}>
                    {CHECKLIST_ITEMS_POR_TIPO[formChecklist.tipo||tipoChecklist].map((item,idx)=>(
                      <Fragment key={item}>
                      <div style={{display:'grid',gridTemplateColumns:'1fr auto auto 1fr',gap:'8px',alignItems:'center',padding:'10px 0',borderBottom:'1px solid #262B38'}}>
                        <span style={{fontSize:'13px',fontWeight:'500',color:'#EEF1F7'}}>{item}</span>
                        <label style={{display:'flex',alignItems:'center',gap:'6px',fontSize:'13px',cursor:'pointer',padding:'6px 12px',borderRadius:'6px',background:formChecklist.items[item]?.valor==='si'?'rgba(46,204,113,.12)':'#1C2130',border:'1px solid',borderColor:formChecklist.items[item]?.valor==='si'?'rgba(46,204,113,.4)':'#343B4B',whiteSpace:'nowrap'}}>
                          ...
                          <span style={{color:formChecklist.items[item]?.valor==='si'?'#2ECC71':'#9AA3B8'}}>✓ Sí</span>
                        </label>
                        <label style={{display:'flex',alignItems:'center',gap:'6px',fontSize:'13px',cursor:'pointer',padding:'6px 12px',borderRadius:'6px',background:formChecklist.items[item]?.valor==='no'?'rgba(255,77,79,.1)':'#1C2130',border:'1px solid',borderColor:formChecklist.items[item]?.valor==='no'?'rgba(255,77,79,.4)':'#343B4B',whiteSpace:'nowrap'}}>
                          ...
                          <span style={{color:formChecklist.items[item]?.valor==='no'?'#FF4D4F':'#9AA3B8'}}>✗ No</span>
                        </label>
                      ...
                      {tipoChecklist==='inyectores'&&item==='FOTO DE CÓDIGO DE INYECTORES CARGADA EN LA PÁGINA'&&(
                        <div style={{padding:'8px 0 14px',borderBottom:'1px solid #262B38',display:'flex',flexDirection:'column',gap:'8px'}}>
                          ...
                          {!formChecklist.trabajo_id&&<span style={{fontSize:'11.5px',color:'#6B7488'}}>Elegí primero el cliente/vehículo arriba para poder subir fotos.</span>}
                          {fotosChecklistSubidas.length>0&&<div style={{display:'flex',gap:'6px',flexWrap:'wrap'}}>{fotosChecklistSubidas.map(f=><img key={f.id} src={f.url} alt="foto" style={{width:'52px',height:'52px',objectFit:'cover',borderRadius:'6px',border:'1px solid #343B4B'}}/>)}</div>}
                        </div>
                      )}
                      </Fragment>
                    ))}
            ...
            {vistaChecklist==='lista'&&(
              <div>
                ...
                {checklistsFiltrados.length===0?<div style={{color:'#6B7488',fontSize:'14px',textAlign:'center',padding:'3rem'}}>{!admin&&!empleadoActual?'Seleccioná tu nombre arriba para ver tus registros':'No hay checklists registrados todavía'}</div>:(
                  <div className={styles.tblWrap}><table className={styles.table}><thead><tr><th>Fecha</th><th>Vehículo</th><th>Patente</th><th>Mecánico</th><th>Acciones</th></tr></thead><tbody>{checklistsFiltrados.slice(0,limiteChecklists).map(ch=>(<tr key={ch.id}><td style={{fontSize:'12px',color:'#9AA3B8'}}>{ch.fecha_entrega?new Date(ch.fecha_entrega+'T12:00:00').toLocaleDateString('es-AR'):'—'}</td><td onClick={()=>setChecklistActivo(ch)} style={{cursor:'pointer'}}><b>{ch.vehiculo||'—'}</b></td><td onClick={()=>setChecklistActivo(ch)} style={{cursor:'pointer'}}>{ch.patente||'—'}</td><td>{ch.mecanico||'—'}</td><td style={{display:'flex',gap:'5px'}}><button className={styles.btn} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>setChecklistActivo(ch)}>Ver</button><button className={styles.btnPrimary} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>abrirEditarChecklist(ch)}>Editar</button><button className={styles.btn} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>imprimirChecklist(ch)}>Imprimir</button>{admin&&<button className={styles.btnDelete} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>borrarChecklist(ch.id)}><TrashIcon/></button>}</td></tr>))}</tbody></table>{checklistsFiltrados.length>limiteChecklists&&<button className={styles.btn} style={{width:'100%',marginTop:'10px'}} onClick={()=>setLimiteChecklists(l=>l+40)}>Ver más ({checklistsFiltrados.length-limiteChecklists} restantes)</button>}</div>
                )}
              </div>
            )}
          </div>
        ...
        {seccion==='empleados'&&admin&&(
          <div>
            <div className={styles.topBar}><h1 className={styles.pageTitle}>Empleados</h1><button className={styles.btn} onClick={actualizarTodo}>↻ Actualizar</button></div>
            <div className={styles.divider}></div>
            ...
            <div className={styles.card}><div className={styles.cardTitle}>Mecánicos</div>{mecanicos.length===0?<div style={{color:'#6B7488',fontSize:'13px'}}>Sin mecánicos</div>:<div style={{display:'flex',flexDirection:'column',gap:'6px'}}>{mecanicos.map(e=><div key={e.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 12px',background:'#1C2130',borderRadius:'6px',border:'1px solid #343B4B'}}><span style={{fontSize:'13px',fontWeight:'500'}}>{e.nombre}</span><button className={styles.btnDelete} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>borrarEmpleado(e.id)}><TrashIcon/></button></div>)}</div>}</div>
          </div>
        )}
        ...
        {seccion==='informe'&&admin&&(
          <div>
            <div className={styles.topBar}><h1 className={styles.pageTitle}>Informe mensual</h1><div style={{display:'flex',gap:'8px',alignItems:'center'}}><button className={styles.btn} onClick={actualizarTodo}>↻ Actualizar</button><input type="month" value={mesInforme} onChange={e=>setMesInforme(e.target.value)} style={{padding:'8px 12px',borderRadius:'6px',border:'1px solid #343B4B',background:'#171B25',color:'#EEF1F7',fontSize:'13px',fontFamily:'inherit'}}/><button className={styles.btnSuccess} onClick={exportarInformeExcel}>Exportar a Excel</button><button className={styles.btnPrimary} onClick={imprimirInforme}>Imprimir informe</button></div></div>
            <div className={styles.divider}></div>
            {(()=>{const{ingresados,salidos,marcaTop,marcasCount,nombreMes}=generarInforme();return<div><div style={{marginBottom:'12px',fontSize:'14px',fontWeight:'600',color:'#9AA3B8',textTransform:'capitalize'}}>{nombreMes}</div><div className={styles.stats} style={{marginBottom:'1.5rem'}}><div className={styles.stat} style={{cursor:'default'}}><div className={styles.statN}>{ingresados.length}</div><div className={styles.statL}>Ingresados</div></div><div className={styles.stat} style={{cursor:'default'}}><div className={styles.statN}>{salidos.length}</div><div className={styles.statL}>Entregados</div></div><div className={styles.stat} style={{cursor:'default'}}><div className={styles.statN} style={{fontSize:'18px',color:'#2ECC71'}}>{marcaTop?marcaTop[0]:'—'}</div><div className={styles.statL}>Marca frecuente{marcaTop?` (${marcaTop[1]})`:''}</div></div><div className={styles.stat} style={{cursor:'default'}}><div className={styles.statN} style={{fontSize:'16px'}}>{Object.keys(marcasCount).length}</div><div className={styles.statL}>Marcas distintas</div></div></div><div className={styles.card}><div className={styles.cardTitle}>Vehículos ingresados ({ingresados.length})</div>{ingresados.length===0?<div style={{color:'#6B7488',fontSize:'13px'}}>Sin ingresos este mes</div>:<table className={styles.table}><thead><tr><th>#</th><th>Vehículo</th><th>Cliente</th><th>Patente</th><th>Taller</th><th>Ingreso</th></tr></thead><tbody>{ingresados.map((t,i)=><tr key={t.id} onClick={()=>verDetalle(t)}><td style={{color:'#6B7488'}}>{i+1}</td><td><b>{t.vehiculos?.marca_modelo}</b></td><td>{t.vehiculos?.clientes?.nombre}</td><td>{t.vehiculos?.patente}</td><td>{t.taller}</td><td style={{fontSize:'12px',color:'#9AA3B8'}}>{new Date(t.fecha_ingreso).toLocaleDateString('es-AR')}</td></tr>)}</tbody></table>}</div><div className={styles.card}><div className={styles.cardTitle}>Vehículos entregados ({salidos.length})</div>{salidos.length===0?<div style={{color:'#6B7488',fontSize:'13px'}}>Sin entregas este mes</div>:<table className={styles.table}><thead><tr><th>#</th><th>Vehículo</th><th>Cliente</th><th>Patente</th><th>Taller</th><th>Entrega</th></tr></thead><tbody>{salidos.map((t,i)=><tr key={t.id} onClick={()=>verDetalle(t)}><td style={{color:'#6B7488'}}>{i+1}</td><td><b>{t.vehiculos?.marca_modelo}</b></td><td>{t.vehiculos?.clientes?.nombre}</td><td>{t.vehiculos?.patente}</td><td>{t.taller}</td><td style={{fontSize:'12px',color:'#9AA3B8'}}>{new Date(t.fecha_salida).toLocaleDateString('es-AR')}</td></tr>)}</tbody></table>}</div><div className={styles.card}><div className={styles.cardTitle}>Marcas atendidas</div><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(130px,1fr))',gap:'8px'}}>{Object.entries(marcasCount).sort((a,b)=>b[1]-a[1]).map(([marca,n])=><div key={marca} style={{background:'#1C2130',borderRadius:'8px',padding:'10px 14px',display:'flex',justifyContent:'space-between',alignItems:'center',border:'1px solid #343B4B'}}><span style={{fontSize:'13px',color:'#9AA3B8',fontWeight:'500'}}>{marca}</span><span style={{fontSize:'20px',fontWeight:'700',color:'#2FA8FF'}}>{n}</span></div>)}{Object.keys(marcasCount).length===0&&<div style={{color:'#6B7488',fontSize:'13px'}}>Sin datos este mes</div>}</div></div></div>})()}
          </div>
        )}

        {seccion==='detalle'&&clienteDetalle&&(
          <div>
            <div className={styles.topBar}><div style={{display:'flex',gap:'8px'}}><button className={styles.btn} onClick={()=>setSeccion('clientes')}>← Volver</button><button className={styles.btn} onClick={actualizarTodo}>↻ Actualizar</button></div><div style={{display:'flex',gap:'8px',flexWrap:'wrap'}}>{admin&&<button className={styles.btn} onClick={()=>imprimirOrden(clienteDetalle)}>Imprimir</button>}{admin&&<button className={styles.btnSuccess} onClick={()=>{setModalActualizar(clienteDetalle);setFormActualizar({tipo:'estado',descripcion:'',taller_nuevo:'Malvinas 3906',mecanico:'',fecha_manual:''})}}>Actualización</button>}{admin&&<button className={styles.btnRepuesto} onClick={()=>setModalRepuesto(clienteDetalle)}>Repuesto</button>}{admin&&<button className={styles.btnSuccess} onClick={()=>abrirWsp(clienteDetalle)}>WhatsApp</button>}{admin&&<button className={styles.btnPrimary} onClick={()=>abrirEditar(clienteDetalle)}>Editar</button>}{admin&&clienteDetalle.estado!=='Salio'&&<button className={styles.btnDangerSolid} onClick={()=>setModalSalida(clienteDetalle)}>Registrar salida</button>}{admin&&clienteDetalle.estado==='Salio'&&<button className={styles.btnPrimary} onClick={()=>{setModalReingreso(clienteDetalle);setFormReingreso({motivo:'',mecanico:clienteDetalle.mecanico||'',taller:clienteDetalle.taller||'Malvinas 2084',estado:'Diagnóstico',llego_en_grua:false,fecha_ingreso_manual:''})}}>Reingreso</button>}{admin&&<button className={styles.btnDanger} onClick={()=>borrarCliente(clienteDetalle)}><TrashIcon/></button>}</div></div>
            <div className={styles.divider}></div>
            <div className={styles.detailHeader}><div className={styles.detailAvatar}>{clienteDetalle.vehiculos?.clientes?.nombre?.charAt(0)}</div><div style={{flex:1}}><div className={styles.detailNombre}>{clienteDetalle.vehiculos?.clientes?.nombre}</div><div className={styles.detailSub}>{clienteDetalle.vehiculos?.clientes?.telefono} · {clienteDetalle.llego_en_grua?'Llegó en grúa':'Llegó andando'} · {clienteDetalle.tiene_seguro?'Con seguro':'Sin seguro'} · N° {clienteDetalle.vehiculos?.clientes?.numero_ficha||'—'}</div></div><span className={badgeClass(clienteDetalle.estado)}>{clienteDetalle.estado}</span></div>

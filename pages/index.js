...
function calcularTiemposTrabajo(trabajo, actualizacionesTrabajo){
  ...
  for(let i=0;i<eventos.length;i++){
    ...
    const siguienteEsTrabajoIniciado=eventos[i+1]?.tipo==='trabajo_iniciado'||eventos[i+1]?.tipo==='reparacion'
...
export default function Home({ rol, cerrarSesion }) {
  ...
  async function guardarActualizacion(){
    ...
    else if(tipo==='trabajo_iniciado')desc=`Trabajo iniciado. ${desc}`
  ...
  const tipoHistorial={ingreso:'🟢',salida:'🔴',movimiento:'🔵',reingreso:'🟡',estado:'⚪',prueba:'🟠',motor:'🔧',terceros:'⏳',repuestos:'📦',aprobacion:'✍️',elevador:'🅿️',diagnostico:'🔍',reparacion:'🛠️',diagnostico_fin:'✅',cliente_aprobo:'👍',cliente_pago:'💵',oficina_salio:'🚗',repuestos_llegaron:'📦',repuesto_no_pactado:'⚠️',mecanico_pide_repuestos:'🔧',trabajo_iniciado:'▶️'}
  ...
  const tiemposDelMes=useMemo(()=>{
    if(seccion!=='tiempos')return{porTrabajo:[],totalesPorCategoria:{},eficiencia:0,tiempoMuertoProm:0,motivoGeneral:null,excluidos:[],statsPorMecanico:[],statsOficina:[],horasClientes:0,horasMuertas:0,horasMuertasSinCerrado:0}
    ...
    return{porTrabajo:porTrabajo.sort((a,b)=>b.totalHoras-a.totalHoras),totalesPorCategoria,eficiencia,tiempoMuertoProm,motivoGeneral,excluidos:excluidosDelMes,statsPorMecanico,statsOficina,horasClientes,horasMuertas,horasMuertasSinCerrado}

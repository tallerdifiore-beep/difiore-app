      ...
      <div className={styles.main}>
        ...
        {seccion==='tiempos'&&admin&&(
          <div>
            ...
            <div className={styles.stats} style={{marginBottom:'1rem'}}>
              ...
              <div className={styles.stat} style={{cursor:'default'}}><div className={styles.statN} style={{color:'#D6363A'}}>{formatHoras(tiemposDelMes.horasClientes)}h</div><div className={styles.statL}>Perdidas por clientes</div></div>
              <div className={styles.stat} style={{cursor:'default'}}>
                {costoHoraTaller>0?<div className={styles.statN} style={{color:'#D6363A',fontSize:'15px'}}>${formatPeso(Math.round(tiemposDelMes.costoTiempoMuerto))}</div>:<div className={styles.statN} style={{fontSize:'13px',color:'#A0AEC0'}}>Sin configurar</div>}
                <div className={styles.statL}>Costo t. muerto{!editandoCostoHora?<span onClick={(e)=>{e.stopPropagation();setCostoHoraTemp(String(costoHoraTaller));setEditandoCostoHora(true)}} style={{display:'block',fontSize:'11px',color:'#1B4CFF',marginTop:'2px',cursor:'pointer'}}>{costoHoraTaller>0?'Cambiar':'Configurar'} $/hora</span>:<div style={{display:'flex',gap:'4px',marginTop:'4px'}} onClick={e=>e.stopPropagation()}><input type="number" value={costoHoraTemp} onChange={e=>setCostoHoraTemp(e.target.value)} style={{width:'70px',padding:'3px 6px',fontSize:'11px',borderRadius:'4px',border:'1px solid #CBD5E0'}}/><button onClick={guardarCostoHoraTaller} style={{fontSize:'11px',padding:'3px 8px',borderRadius:'4px',border:'none',background:'#1B4CFF',color:'#fff',cursor:'pointer'}}>OK</button></div>}</div>
              </div>
            </div>
            ...
            {trabajoSeleccionadoTiempos?(
              <div className={styles.card}>
                {(()=>{
                  ...
                  return(<>
                    <div className={styles.cardTitle}>{trabajoSeleccionadoTiempos.vehiculos?.marca_modelo}{esReingreso&&<span style={{marginLeft:'8px',fontSize:'11px',fontWeight:'700',color:'#1B4CFF',background:'#EFF6FF',padding:'3px 9px',borderRadius:'20px'}}>Reingreso</span>}</div>
            ...
            {vistaTiempos==='terceros'&&(
              <div className={styles.card}>
                ...
                {tiemposDelMes.porTerceros.length>0&&<table className={styles.table}><thead><tr><th>Vehículo</th><th>Cliente</th><th>Horas</th></tr></thead><tbody>{tiemposDelMes.porTerceros.map(({trabajo,horas})=>(<tr key={trabajo.id} onClick={()=>setTrabajoSeleccionadoTiempos(trabajo)}><td><b>{trabajo.vehiculos?.marca_modelo}</b></td><td>{trabajo.vehiculos?.clientes?.nombre}</td><td style={{fontFamily:'monospace',color:'#D6363A'}}>{formatHoras(horas)}h</td></tr>))}</tbody></table>}
              </div>
            )}
            ...
            {vistaTiempos==='cliente'&&(
              <div className={styles.card}>
                ...
                {tiemposDelMes.porCliente.length>0&&<table className={styles.table}><thead><tr><th>Cliente</th><th>Vehículo</th><th>Horas</th></tr></thead><tbody>{tiemposDelMes.porCliente.map(({trabajo,horas})=>(<tr key={trabajo.id} onClick={()=>setTrabajoSeleccionadoTiempos(trabajo)}><td><b>{trabajo.vehiculos?.clientes?.nombre}</b></td><td>{trabajo.vehiculos?.marca_modelo}</td><td style={{fontFamily:'monospace',color:'#D6363A'}}>{formatHoras(horas)}h</td></tr>))}</tbody></table>}
              </div>
            )}

            {vistaTiempos==='historico'&&(<>
              {cuelloBotellaPersistente&&<div className={styles.card} style={{background:'#FEF2F2',border:'1px solid #FECACA'}}>
                <div style={{fontSize:'13px',color:'#991B1B',fontWeight:'600'}}>"{cuelloBotellaPersistente}" es el motivo principal hace 3 meses seguidos — parece un problema estructural, no un mes suelto.</div>
              </div>}
              <div className={styles.card}>
              ...
              <div className={styles.card}>
                ...
                <table className={styles.table}><thead><tr><th>Mes</th><th>Motivo principal</th><th>Vehículos</th></tr></thead><tbody>{historicoTiempos.map(m=>(<tr key={m.mes}><td style={{textTransform:'capitalize'}}>{new Date(m.mes+'-15').toLocaleDateString('es-AR',{month:'long',year:'numeric'})}</td><td style={{color:'#D6363A'}}>{m.motivoGeneral?m.motivoGeneral[0]:'—'}</td><td style={{fontFamily:'monospace'}}>{m.cantidadVehiculos}</td></tr>))}</tbody></table>
              </div>
            </>)}
            ...
            {planPorMecanico.map(([mecanico,tareas])=>(
              <div key={mecanico} className={styles.card}>
                <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'12px'}}>
                  <div style={{width:'26px',height:'26px',borderRadius:'50%',background:'#EFF6FF',color:'#1B4CFF',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'11px',fontWeight:'700'}}>{mecanico==='Sin asignar'?'?':mecanico.split(' ').map(w=>w[0]).slice(0,2).join('').toUpperCase()}</div>
        ...
        {estancadosExcluidos.length>0&&<div className={styles.card} style={{marginTop:'1rem'}}>
          ...
          {mostrarExcluidosEstancados&&<>
            ...
            <table className={styles.table}><thead><tr><th>Vehículo</th><th>Cliente</th><th>Días</th><th></th></tr></thead><tbody>{estancadosExcluidos.map(t=>(<tr key={t.id}><td onClick={()=>verDetalle(t)}><b>{t.vehiculos?.marca_modelo}</b></td><td onClick={()=>verDetalle(t)}>{t.vehiculos?.clientes?.nombre}</td><td onClick={()=>verDetalle(t)} style={{color:'#718096'}}>{diasDesde(fechaReferenciaEstancado(t))} días</td><td style={{cursor:'default'}}><button className={styles.btn} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>toggleExcluirEstancados(t)}>Volver a incluir</button></td></tr>))}</tbody></table>
          </>}
        </div>}
        </div>)}

        {seccion==='dashboard'&&!tallerVista&&vistaStats==='reingresos'&&(<div><div className={styles.topBar}><button className={styles.btn} onClick={()=>setVistaStats(null)}>← Volver</button><h1 className={styles.pageTitle}>Reingresos</h1></div><div className={styles.divider}></div><div className={styles.tblWrap}><table className={styles.table}><thead><tr><th>#</th><th>Vehículo</th><th>Cliente</th><th>Patente</th><th>Veces que reingresó</th></tr></thead><tbody>{reingresosPorVehiculo.map((r,i)=><tr key={r.vehiculo?.id||i} onClick={()=>{const t=trabajos.find(tr=>tr.vehiculos?.id===r.vehiculo?.id);if(t)verDetalle(t)}}><td style={{color:'#A0AEC0'}}>{i+1}</td><td><b>{r.vehiculo?.marca_modelo}</b></td><td>{r.vehiculo?.clientes?.nombre}</td><td>{r.vehiculo?.patente}</td><td style={{fontWeight:'700',color:'#1B4CFF'}}>{r.cantidad} {r.cantidad===1?'vez':'veces'}</td></tr>)}{reingresosPorVehiculo.length===0&&<tr><td colSpan="5" style={{textAlign:'center',color:'#A0AEC0',padding:'2rem'}}>Sin reingresos registrados</td></tr>}</tbody></table></div></div>)}
            ...
            {verRecordatorios&&(<div className={styles.card} style={{marginBottom:'1rem'}}>
              ...
              {turnosManiana.map(t=>(
                <div key={t.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'10px 12px',background:'#F7FAFC',borderRadius:'8px',border:'1px solid #E2E8F0',marginBottom:'8px'}}>
                  ...
                  {t.telefono?<button style={{fontSize:'12px',padding:'6px 12px',background:'rgba(46,204,113,.12)',color:'#2ECC71',border:'1px solid rgba(46,204,113,.4)',borderRadius:'6px',cursor:'pointer',fontFamily:"'Barlow Condensed',sans-serif",fontWeight:'700',textTransform:'uppercase',letterSpacing:'.03em'}} onClick={()=>enviarRecordatorioTurno(t)}>Enviar</button>:<span style={{fontSize:'11px',color:'#A0AEC0'}}>Sin teléfono</span>}
                </div>
              ))}
            </div>)}
            <div className={styles.divider}></div>
                ...
                <div style={{display:'flex',flexDirection:'column',gap:'1rem'}}>
                  <div className={styles.card}>
                    ...
                    {turnosPorDia(diaSeleccionado).map(t=>{
                      ...
                      const estadoInfo={
                        pendiente:{label:'Pendiente',bg:'#E5E8F0',color:'#4A5568'},
                        presento:{label:'Se presentó',bg:'rgba(46,204,113,.15)',color:'#1D9754'},
                        no_presento:{label:'No se presentó',bg:'rgba(255,77,79,.15)',color:'#D6363A'},
                      ...
                      <div key={t.id} style={{padding:'10px 12px',background:'#F7FAFC',borderRadius:'8px',border:'1px solid #E2E8F0',marginBottom:'8px'}}>
                        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                          ...
                          <div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:'4px'}}>
                            <div style={{fontSize:'11px',fontWeight:'700',color:'#1B4CFF'}}>8:30 hs</div>
                        ...
                        <div style={{display:'flex',gap:'4px',marginTop:'6px',flexWrap:'wrap',borderTop:'1px solid #E2E8F0',paddingTop:'6px'}}>
                          <button style={{fontSize:'10px',padding:'3px 7px',background:estadoTurno==='presento'?'#2ECC71':'#E5E8F0',color:estadoTurno==='presento'?'#0B0D12':'#4A5568',border:'1px solid #C9D0DD',borderRadius:'6px',cursor:'pointer',fontFamily:"'Barlow Condensed',sans-serif",fontWeight:'700',textTransform:'uppercase',letterSpacing:'.03em'}} onClick={()=>usarTurnoComoNuevoCliente(t)}>Se presentó</button>
                          <button style={{fontSize:'10px',padding:'3px 7px',background:estadoTurno==='no_presento'?'#FF4D4F':'#E5E8F0',color:estadoTurno==='no_presento'?'#fff':'#4A5568',border:'1px solid #C9D0DD',borderRadius:'6px',cursor:'pointer',fontFamily:"'Barlow Condensed',sans-serif",fontWeight:'700',textTransform:'uppercase',letterSpacing:'.03em'}} onClick={()=>marcarEstadoTurno(t,'no_presento')}>No se presentó</button>
                          <button style={{fontSize:'10px',padding:'3px 7px',background:estadoTurno==='reprogramado'?'#F5B700':'#E5E8F0',color:estadoTurno==='reprogramado'?'#0B0D12':'#4A5568',border:'1px solid #C9D0DD',borderRadius:'6px',cursor:'pointer',fontFamily:"'Barlow Condensed',sans-serif",fontWeight:'700',textTransform:'uppercase',letterSpacing:'.03em'}} onClick={()=>{marcarEstadoTurno(t,'reprogramado');abrirEditarTurno(t)}}>Reprogramó</button>
                        </div>
                      </div>
                      )
                    })}
          ...
          <div>
            ...
            {vistaChecklist==='procedimiento'&&(
              <div className={styles.card}>
                ...
                <div style={{background:'#FEF3C7',border:'1px solid #FDE68A',borderRadius:'8px',padding:'12px 14px',marginBottom:'14px',fontSize:'13px',color:'#92400E',fontWeight:'600'}}>Cuando hay trabajos de inyectores, los hace una sola persona por completo.</div>
                <div style={{display:'flex',flexDirection:'column',gap:'0'}}>
                  {PROCEDIMIENTO_INYECTORES.map((paso,idx)=>(
                    <div key={idx} style={{display:'flex',gap:'14px',padding:'14px 0',borderBottom:idx<PROCEDIMIENTO_INYECTORES.length-1?'1px solid #EDF2F7':'none'}}>
                      <div style={{width:'28px',height:'28px',borderRadius:'50%',background:'#EFF6FF',color:'#1B4CFF',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'13px',fontWeight:'700',flexShrink:0}}>{idx+1}</div>
                      <div>

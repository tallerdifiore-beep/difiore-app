  ...
  const checklistsFiltrados = (() => {
    ...
    if (busquedaChecklist.trim()) {
      ...
      lista = lista.filter(ch =>
        ch.vehiculo?.toLowerCase().includes(q) ||
        ch.patente?.toLowerCase().includes(q) ||
        ch.mecanico?.toLowerCase().includes(q)
      )
    }
    return lista
  })()
          ...
          {celdas.map((dia,i)=>{
            ...
            return(
              <div key={i} onClick={()=>abrirDia(f)} style={{
                minHeight:'72px',borderRadius:'10px',padding:'6px',cursor:'pointer',
            ...
            {vistaChecklist==='nuevo'&&(
              <div>
                ...
                <div className={styles.formActions}>
                  ...
                  <button className={styles.btnPrimary} onClick={guardarChecklist}>{editandoChecklist?'Actualizar checklist':'Guardar checklist'}</button>
                </div>
              </div>
            )}
            ...
            {vistaChecklist==='lista'&&(
              <div>
                <div className={styles.searchBar} style={{marginBottom:'12px'}}>
                  <input type="text" placeholder="Buscar por vehículo, patente o mecánico..." value={busquedaChecklist} onChange={e=>setBusquedaChecklist(e.target.value)}/>
                </div>
                {checklistsFiltrados.length===0?<div style={{color:'#A0AEC0',fontSize:'14px',textAlign:'center',padding:'3rem'}}>{!admin&&!empleadoActual?'Seleccioná tu nombre arriba para ver tus registros':'No hay checklists registrados todavía'}</div>:(
                  <div className={styles.tblWrap}><table className={styles.table}><thead><tr><th>Fecha</th><th>Vehículo</th><th>Patente</th><th>Mecánico</th><th>Acciones</th></tr></thead><tbody>{checklistsFiltrados.map(ch=>(<tr key={ch.id}><td style={{fontSize:'12px',color:'#718096'}}>{ch.fecha_entrega?new Date(ch.fecha_entrega+'T12:00:00').toLocaleDateString('es-AR'):'—'}</td><td onClick={()=>setChecklistActivo(ch)} style={{cursor:'pointer'}}><b>{ch.vehiculo||'—'}</b></td><td onClick={()=>setChecklistActivo(ch)} style={{cursor:'pointer'}}>{ch.patente||'—'}</td><td>{ch.mecanico||'—'}</td><td style={{display:'flex',gap:'5px'}}><button className={styles.btn} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>setChecklistActivo(ch)}>Ver</button><button className={styles.btnPrimary} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>abrirEditarChecklist(ch)}>✏️ Editar</button><button className={styles.btn} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>imprimirChecklist(ch)}>🖨️</button>{admin&&<button className={styles.btnDelete} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>borrarChecklist(ch.id)}>🗑️</button>}</td></tr>))}</tbody></table></div>
                )}
              </div>
            )}
          </div>

...
export default function Home({ rol, cerrarSesion }) {
  ...
  const tipoHistorial={ingreso:'🟢',salida:'🔴',movimiento:'🔵',reingreso:'🟡',estado:'⚪',prueba:'🟠',motor:'🔧',terceros:'⏳',repuestos:'📦',aprobacion:'✍️',elevador:'🅿️',diagnostico:'🔍',reparacion:'🛠️',diagnostico_fin:'✅',cliente_aprobo:'👍',cliente_pago:'💵',oficina_salio:'🚗',repuestos_llegaron:'📦',repuesto_no_pactado:'⚠️',mecanico_pide_repuestos:'🔧'}
...
{seccion==='recibo'&&admin&&(
        ...
        {seccion==='detalle'&&clienteDetalle&&(
          <div>
            ...
            {(()=>{const mecs=[...new Set(historial.filter(h=>h.mecanico).map(h=>h.mecanico))];return mecs.length>0&&<div className={styles.card}><div className={styles.cardTitle}>Mecánicos que intervinieron</div><div style={{display:'flex',gap:'6px',flexWrap:'wrap'}}>{mecs.map(m=><span key={m} style={{fontSize:'12px',padding:'4px 12px',borderRadius:'20px',background:m==='Oficina'?'#EFF6FF':'#F7FAFC',color:m==='Oficina'?'#2563EB':'#4A5568',fontWeight:'600'}}>{m==='Oficina'?'🏢 ':''}{m}</span>)}</div></div>})()}

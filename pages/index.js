...
export default function Home({ rol, cerrarSesion }) {
  ...
  useEffect(() => { cargarDatos(); cargarNumeracion(); cargarPresupuestos(); cargarReingresos(); cargarPlanDiario(fechaPlan); cargarActualizacionesGlobal(); cargarUmbralEstancados() }, [])
  ...
  const trabajosEntregadosFiltrados=trabajosEntregados.filter(t=>{const q=busquedaEntregados.toLowerCase();return t.vehiculos?.clientes?.nombre?.toLowerCase().includes(q)||t.vehiculos?.patente?.toLowerCase().includes(q)||t.vehiculos?.marca_modelo?.toLowerCase().includes(q)})
  ...
  const trabajosEstancados=trabajosActivos.filter(t=>diasDesde(fechaReferenciaEstancado(t))>=umbralEstancados&&!t.excluir_estancados).sort((a,b)=>diasDesde(fechaReferenciaEstancado(b))-diasDesde(fechaReferenciaEstancado(a)))
  const estancadosExcluidos=trabajosActivos.filter(t=>diasDesde(fechaReferenciaEstancado(t))>=umbralEstancados&&t.excluir_estancados).sort((a,b)=>diasDesde(fechaReferenciaEstancado(b))-diasDesde(fechaReferenciaEstancado(a)))

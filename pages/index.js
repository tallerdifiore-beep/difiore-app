...
export default function Home({ rol, cerrarSesion }) {
  ...
  async function cargarFotos(id) {
    const {data,error} = await supabase.from('fotos').select('*').eq('trabajo_id',id).order('created_at',{ascending:false})
    if(!error) setFotos(data||[])
  }
  async function cargarFotosModal(id) {
    const {data,error} = await supabase.from('fotos').select('*').eq('trabajo_id',id).order('created_at',{ascending:false})
    if(!error) setModalFotosData(data||[])
  }
  async function cargarHistorial(id) {
    const {data:h1} = await supabase.from('historial').select('*').eq('trabajo_id',id)
    const {data:h2} = await supabase.from('actualizaciones').select('*').eq('trabajo_id',id)
    setHistorial([...(h1||[]),...(h2||[])].sort((a,b)=>new Date(b.fecha)-new Date(a.fecha)))
  }
  async function cargarRepuestos(id) {
    const {data} = await supabase.from('repuestos').select('*').eq('trabajo_id',id).order('fecha',{ascending:false})
    setRepuestos(data||[])
  }
...
<script>window.onload=()=>{window.print()}<\/script>
  ...
  function badgeClass(e){
    ...
    if(e==='En espera') return styles.badgeBlue
    if(e==='Desarmando') return styles.badgeRed
                ...
                return <div key={taller} className={styles.tallerCard} onClick={()=>setTallerVista(taller)}>
                  ...
                  <div style={{marginTop:'10px'}}>
                    {['Diagnóstico','En proceso','En espera','Desarmando','Listo'].map(estado=>{
                      ...
                      return n>0?<div key={estado} style={{display:'flex',justifyContent:'space-between',fontSize:'12px',padding:'4px 0',borderBottom:'1px solid #EDF2F7'}}><span style={{color:'#718096'}}>{estado}</span><span style={{color:'#2D3748',fontWeight:'600'}}>{n}</span></div>:null
                    })}
                  </div>
        ...
        {seccion==='dashboard'&&!tallerVista&&vistaStats&&(
          <div>
            ...
            <div className={styles.tblWrap}><table className={styles.table}>
              <thead><tr><th>#</th><th>Vehículo</th><th>Cliente</th><th>Patente</th><th>Estado</th><th>Taller</th><th>Ingreso</th></tr></thead>
              <tbody>
                {listaVistaStats[vistaStats].map((t,i)=><tr key={t.id} onClick={()=>verDetalle(t)}><td style={{color:'#A0AEC0'}}>{i+1}</td><td><b>{t.vehiculos?.marca_modelo}</b></td><td>{t.vehiculos?.clientes?.nombre}</td><td>{t.vehiculos?.patente}</td><td><span className={badgeClass(t.estado)}>{t.estado}</span></td><td>{t.taller}</td><td style={{fontSize:'12px',color:'#718096'}}>{new Date(t.fecha_ingreso).toLocaleDateString('es-AR')}</td></tr>)}
            ...
            <form onSubmit={guardarCliente}>
              ...
              <div className={styles.card}>
                ...
                <div className={styles.formGrid}>
                  ...
                  <div className={styles.formGroup} style={{gridColumn:'1/-1'}}><label>Motivo de ingreso</label><textarea value={form.motivo} onChange={e=>setForm({...form,motivo:e.target.value})} placeholder="Reparación de motor. Cambio de distribución. Service completo."/></div>
                  ...
                  <div className={styles.formGroup}><label>Llegó en</label><select value={form.llego_en_grua?'grua':'andando'} onChange={e=>setForm({...form,llego_en_grua:e.target.value==='grua'})}><option value="andando">Andando</option><option value="grua">En grúa</option></select></div>
                  <div className={styles.formGroup}><label>Tiene seguro</label><select value={form.tiene_seguro?'si':'no'} onChange={e=>setForm({...form,tiene_seguro:e.target.value==='si'})}><option value="no">No</option><option value="si">Sí</option></select></div>
                  ...
                  <div className={styles.formGroup}><label>Estado</label><select value={form.estado} onChange={e=>setForm({...form,estado:e.target.value})}><option>Diagnóstico</option><option>En proceso</option><option>En espera</option><option>Desarmando</option><option>Listo</option></select></div>
                  <div className={styles.formGroup} style={{gridColumn:'1/-1'}}><label>Taller</label><select value={form.taller} onChange={e=>setForm({...form,taller:e.target.value})}><option>Malvinas 2084</option><option>Malvinas 3906</option></select></div>
                </div>
              </div>
              <div className={styles.card}>
                ...
                <button type="button" className={styles.btnPrimary} onClick={()=>fileNuevoRef.current.click()}>{fotoNuevo.length>0?`✓ ${fotoNuevo.length} foto(s)`:'+ Seleccionar fotos'}</button>
                {fotoNuevo.length>0&&<div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'8px',marginTop:'12px'}}>
                  {fotoNuevo.map((f,i)=><div key={i} style={{position:'relative'}}><img src={URL.createObjectURL(f)} alt="preview" style={{width:'100%',aspectRatio:'4/3',objectFit:'cover',borderRadius:'8px'}}/><button type="button" className={styles.fotoBorrar} onClick={()=>setFotoNuevo(fotoNuevo.filter((_,j)=>j!==i))}>✕</button></div>)}
                </div>}
              </div>
                  ...
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'10px'}}>
                    <div style={{display:'flex',alignItems:'center',gap:'12px'}}>
                      ...
                      <label style={{display:'flex',alignItems:'center',gap:'5px',fontSize:'12px',cursor:'pointer',color:'#2563EB',fontWeight:'500'}}>
                        <input type="checkbox" checked={item.es_mano_obra} onChange={e=>{const items=[...presupuesto.items];items[idx]={...items[idx],es_mano_obra:e.target.checked};setPresupuesto({...presupuesto,items})}}/> Es mano de obra
                      </label>
                    </div>
            ...
            {(()=>{
              ...
              return <div>
                ...
                <div className={styles.stats} style={{marginBottom:'1.5rem'}}>
                  ...
                  <div className={styles.stat} style={{cursor:'default'}}><div className={styles.statN} style={{fontSize:'18px',color:'#16A34A'}}>{marcaTop?marcaTop[0]:'—'}</div><div className={styles.statL}>Marca frecuente{marcaTop?` (${marcaTop[1]})`:''}</div></div>
          ...
          <div>
            ...
            <div className={styles.detGrid}>
              <div className={styles.card}>
                ...
                {[['Modelo',clienteDetalle.vehiculos?.marca_modelo],['Patente',clienteDetalle.vehiculos?.patente],['Color',clienteDetalle.vehiculos?.color],['Año',clienteDetalle.vehiculos?.anio],['Km',clienteDetalle.vehiculos?.kilometraje],['Mecánico',clienteDetalle.mecanico],['Taller',clienteDetalle.taller],['Seguro',clienteDetalle.tiene_seguro?'Sí':'No']].map(([k,v])=>(
            ...
            <div className={styles.card}>
              ...
              {repuestos.length>0&&<table className={styles.table}>

...
{seccion==='recibo'&&admin&&(
        ...
        {seccion==='informe'&&admin&&(
          <div>
            <div className={styles.topBar}><h1 className={styles.pageTitle}>Informe mensual</h1><div style={{display:'flex',gap:'8px',alignItems:'center'}}><button className={styles.btn} onClick={actualizarTodo}>↻ Actualizar</button><input type="month" value={mesInforme} onChange={e=>setMesInforme(e.target.value)} style={{padding:'8px 12px',borderRadius:'6px',border:'1px solid #CBD5E0',fontSize:'13px',fontFamily:'inherit'}}/><button style={{padding:'8px 16px',borderRadius:'6px',fontSize:'13px',cursor:'pointer',background:'#15803D',color:'#fff',border:'none',fontFamily:'inherit',fontWeight:'600'}} onClick={exportarInformeExcel}>Exportar a Excel</button><button className={styles.btnPrimary} onClick={imprimirInforme}>Imprimir informe</button></div></div>
            <div className={styles.divider}></div>

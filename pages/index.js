...
function formatPatente(val) {
  const p = (val||'').toUpperCase().replace(/[^A-Z0-9]/g,'').slice(0,7)
                ...
                {tareas.map(p=>{
                  ...
                  return (
                    <div key={p.id} style={{display:'flex',gap:'10px',alignItems:'flex-start',padding:'8px 0',borderBottom:'1px solid #262B38'}}>
                      ...
                      {admin&&<button className={styles.btnDelete} style={{fontSize:'10px',padding:'3px 7px'}} onClick={()=>borrarTareaPlan(p)}>Borrar</button>}
                    </div>
                  )
                })}
              </div>
                    ...
                    {turnosPorDia(diaSeleccionado).map(t=>{
                      ...
                      <div key={t.id} style={{padding:'10px 12px',background:'#1C2130',borderRadius:'8px',border:'1px solid #343B4B',marginBottom:'8px'}}>
                        ...
                        <div style={{display:'flex',gap:'4px',marginTop:'8px',flexWrap:'wrap'}}>
                          ...
                          <button className={styles.btnDelete} style={{fontSize:'11px',padding:'3px 8px'}} onClick={()=>borrarTurno(t.id)}>Borrar</button>
                        </div>
                      ...
                      {presupuestosFiltrados.map(p=>(
                        <tr key={p.id}>
                          ...
                          <td style={{display:'flex',gap:'5px',cursor:'default'}}>
                            ...
                            <button className={styles.btnDelete} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>borrarPresupuesto(p)}>Borrar</button>
                          </td>
                        </tr>
                      ))}
              ...
              <div>
                <div className={styles.card}>
                  ...
                  <div className={styles.formGrid}>
                    ...
                    <div className={styles.formGroup}><label>Patente</label><input value={formChecklist.patente} onChange={e=>setFormChecklist({...formChecklist,patente:formatPatente(e.target.value)})} placeholder="AB 123 CD"/></div>
            ...
            {vistaChecklist==='lista'&&(
              <div>
                ...
                {checklistsFiltrados.length===0?<div style={{color:'#6B7488',fontSize:'14px',textAlign:'center',padding:'3rem'}}>{!admin&&!empleadoActual?'Seleccioná tu nombre arriba para ver tus registros':'No hay checklists registrados todavía'}</div>:(
                  <div className={styles.tblWrap}><table className={styles.table}><thead><tr><th>Fecha</th><th>Vehículo</th><th>Patente</th><th>Mecánico</th><th>Acciones</th></tr></thead><tbody>{checklistsFiltrados.slice(0,limiteChecklists).map(ch=>(<tr key={ch.id}><td style={{fontSize:'12px',color:'#9AA3B8'}}>{ch.fecha_entrega?new Date(ch.fecha_entrega+'T12:00:00').toLocaleDateString('es-AR'):'—'}</td><td onClick={()=>setChecklistActivo(ch)} style={{cursor:'pointer'}}><b>{ch.vehiculo||'—'}</b></td><td onClick={()=>setChecklistActivo(ch)} style={{cursor:'pointer'}}>{ch.patente||'—'}</td><td>{ch.mecanico||'—'}</td><td style={{display:'flex',gap:'5px'}}><button className={styles.btn} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>setChecklistActivo(ch)}>Ver</button><button className={styles.btnPrimary} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>abrirEditarChecklist(ch)}>Editar</button><button className={styles.btn} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>imprimirChecklist(ch)}>Imprimir</button>{admin&&<button className={styles.btnDelete} style={{fontSize:'11px',padding:'4px 8px'}} onClick={()=>borrarChecklist(ch.id)}>Borrar</button>}</td></tr>))}</tbody></table>{checklistsFiltrados.length>limiteChecklists&&<button className={styles.btn} style={{width:'100%',marginTop:'10px'}} onClick={()=>setLimiteChecklists(l=>l+40)}>Ver más ({checklistsFiltrados.length-limiteChecklists} restantes)</button>}</div>
                )}
              </div>
            )}
          </div>

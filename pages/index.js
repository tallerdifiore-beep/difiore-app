...
export default function Home({ rol, cerrarSesion }) {
  ...
  async function registrarSalida(){
    ...
    const fechaSalida=fechaSalidaManual?new Date(fechaSalidaManual+'T12:00:00').toISOString():new Date().toISOString()
...
return (
    <div className={styles.app}>
      ...
      {modalSalida&&<div className={styles.modalOverlay}><div className={styles.modal}><div className={styles.modalTitle}>Registrar salida</div><div className={styles.modalSub}><b>{modalSalida.vehiculos?.marca_modelo}</b> — {modalSalida.vehiculos?.clientes?.nombre}</div><div className={styles.formGroup} style={{marginTop:'1rem'}}><label>Observación final</label><textarea value={observacionFinal} onChange={e=>setObservacionFinal(e.target.value)} placeholder="Trabajo realizado, recomendaciones, etc..."/></div><div className={styles.formGroup} style={{marginTop:'10px'}}><label>Fecha de salida</label><input type="date" value={fechaSalidaManual} onChange={e=>setFechaSalidaManual(e.target.value)}/><div style={{fontSize:'11px',color:'#A0AEC0',marginTop:'4px'}}>Dejalo vacío para usar la fecha de hoy ({new Date().toLocaleDateString('es-AR')}).</div></div><div className={styles.modalActions}><button className={styles.btn} onClick={()=>{setModalSalida(null);setFechaSalidaManual('')}}>Cancelar</button><button className={styles.btnDangerSolid} onClick={registrarSalida} disabled={guardandoSalida}>{guardandoSalida?'Guardando...':'Confirmar salida'}</button></div></div></div>}
      ...
      <div className={styles.main}>
        ...
        {seccion==='turnos'&&admin&&(
          <div>
            <div className={styles.topBar}><h1 className={styles.pageTitle}>📅 Turnos</h1><div style={{display:'flex',gap:'8px'}}><button className={styles.btn} onClick={actualizarTodo}>↻ Actualizar</button><button className={styles.btnPrimary} onClick={()=>{setMostrarFormTurno(true);setEditandoTurno(null);setFormTurno({nombre:'',telefono:'',vehiculo:'',fecha:diaSeleccionado||'',motivo:''})}}>+ Nuevo turno</button></div></div>
            <div className={styles.divider}></div>
...
{seccion==='recibo'&&admin&&(
        ...
        {seccion==='informe'&&admin&&(
          <div>
            <div className={styles.topBar}><h1 className={styles.pageTitle}>Informe mensual</h1><div style={{display:'flex',gap:'8px',alignItems:'center'}}><button className={styles.btn} onClick={actualizarTodo}>↻ Actualizar</button><input type="month" value={mesInforme} onChange={e=>setMesInforme(e.target.value)} style={{padding:'8px 12px',borderRadius:'6px',border:'1px solid #CBD5E0',fontSize:'13px',fontFamily:'inherit'}}/><button className={styles.btnPrimary} onClick={imprimirInforme}>🖨️ Imprimir informe</button></div></div>
            <div className={styles.divider}></div>
